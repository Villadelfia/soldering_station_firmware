...
// Map X, which is in terms of the range (in_min; in_max) to the range (out_min; out_max)
...
float tempLUT[] = {
    ...
    22.776, 22.819, 22.862, 22.904, 22.947, 22.990, 23.032, 23.075, 23.117, 23.160, 99.999
};
...
int16_t rawTemperatureToC(uint16_t raw, uint16_t coldJunctionTemp) {
    ...
    // The opamp is designed to amplify the voltage of the thermocouple with a
    // factor of 181. We reverse this here.
    mv /= 181;
    
    // Subtract the cold junction voltage from the measured voltage.
    mv -= tempLUT[coldJunctionTemp];
    
    // Look up the temperature in the table.
    int16_t temp = 0;
    while(tempLUT[temp] < mv) temp++;
    return temp;
}

// Gets the approximate temperature of the MCU.
uint16_t getDieTemperature(void) {
    // Set ADC to 10 bits.
    ADC0.CTRLA = ADC_RESSEL_10BIT_gc | ADC_ENABLE_bm;
    
    // Configure amount of samples.
    ADC0.CTRLB = ADC_SAMPNUM_ACC1_gc;
    
    // Set the ADC clock.
    ADC0.CTRLC = ADC_PRESC_DIV16_gc;
    
    // Internal VREF to 1.1V.
    ...
    // Initial delay of 32 samples.
    ADC0.CTRLD = ADC_INITDLY_DLY32_gc;
    
    // Set a sample time of ? 32 µs (33 here).
    ADC0.SAMPCTRL = 31;
    
    // Enable the sample capacitance.
    ...
    // Each chip has its own calibration data burned into the chip.
    int8_t tempOffset = SIGROW.TEMPSENSE1;
    uint8_t tempGain = SIGROW.TEMPSENSE0;
    
    // Apply the offset.
    uint32_t temp = result - tempOffset;
    ...
    // The result is in kelvin, so convert to celsius.
